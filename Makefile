...
perimeter-health:
	curl -fsS -H 'Host: app.aquarium.local' http://127.0.0.1:$(PERIMETER_HTTP_PORT)/auth/login/ >/dev/null
	curl -fsS -H 'Host: auth.aquarium.local' http://127.0.0.1:$(PERIMETER_HTTP_PORT)/ >/dev/null
	curl -fsS -H 'Host: grafana.aquarium.local' http://127.0.0.1:$(PERIMETER_HTTP_PORT)/ >/dev/null
	curl -fsS -H 'Host: secrets.aquarium.local' http://127.0.0.1:$(PERIMETER_HTTP_PORT)/ >/dev/null
